...
* **Women's health types** (#16): authorize and sync the remaining HealthKit reproductive types that were missing from `HealthDataType` — `basalBodyTemperature` as a quantity sample, plus `intermenstrualBleeding`, `infrequentMenstrualCycles`, `irregularMenstrualCycles`, `persistentIntermenstrualBleeding`, `prolongedMenstrualPeriods`, `progesteroneTestResult`, `contraceptive`, `pregnancy`, `pregnancyTestResult`, and `lactation` as category samples. They ride in `records[]` with the existing record shape (`type` is the HealthKit identifier, `value` is the category enum or °C). The four types that were already exported (`menstrualFlow`, `cervicalMucusQuality`, `ovulationTestResult`, `sexualActivity`) are unchanged. Cycle-deviation types are iOS 16+. The backend does not map the category identifiers yet, so they will be accepted on `/sync` and skipped at import until that lands.
